	...
	$('#empNoCheck').on('click', function (evt) {
		...
		$.ajax({
			...
			success: function (result) {
				...
				// 아이디 유효성 검사
				if (result === "2") {
					empNoCheckResult.css('color', '#ff5555'); // 밝은 빨간색 (실패/중복)
					empNoCheckResult.text("해당 사원ID는 이미 가입되었습니다.");
					formIdCheck = false;
				} else if (result === "3") {
					empNoCheckResult.css('color', '#ffaa00'); // 밝은 주황색 (불일치)
					empNoCheckResult.text("사원ID와 이름이 일치하지 않습니다.");
					formIdCheck = false;
				} else if (result === "1") {
					empNoCheckResult.css('color', '#9CDCFE'); // 밝은 하늘색 (성공)
					empNoCheckResult.text("사원ID가 확인되었습니다.");
					formIdCheck = true;
				} else if (result === "4") {
					$("#empEmailDiv").css('display', 'none');
					$("#empEmail").removeAttr('required');
					empNoCheckResult.css('color', '#9CDCFE'); // 밝은 하늘색 (카카오 연동 성공)
					empNoCheckResult.text("카카오ID 연동 시작.");
					formIdCheck = true;
				} else {
					// 예상치 못한 결과값 처리
					empNoCheckResult.css('color', '#ff5555'); // 밝은 빨간색 (오류)
