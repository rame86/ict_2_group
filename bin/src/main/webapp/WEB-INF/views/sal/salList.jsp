						...
						<c:if test="${not empty summary}">
							<div class="sal-summary-row">
								...
								<div class="summary-card">
									...
									<div class="summary-value">
										<fmt:formatNumber value="${summary.ytdRealPay}"
											pattern="#,##0" />
										원
									</div>
								</div>
							...
							<table id="salTable" class="salary-table display">
								...
								<tbody>
									<c:forEach var="sal" items="${salList}">
										<tr>
											<!-- 지급월 -->
											<td><a href="#" class="month-link month-pill"
												data-empno="${sal.empNo}"
												data-monthattno="${sal.monthAttno}">
													${sal.yearMonthLabel} </a> <c:choose>
											...
											<!-- 총 지급액 -->
											<td class="pay-total"><fmt:formatNumber
													value="${sal.payTotal}" pattern="#,##0" />원</td>

											<!-- 지급 요약 -->
											<td class="allowance-cell">
												<div class="allowance-line">
													<span>초과근무</span> <span><fmt:formatNumber
															value="${sal.overtimePay}" pattern="#,###" />원</span>
												</div>
												<div class="allowance-line">
													<span>성과급</span> <span><fmt:formatNumber
															value="${sal.salBonus}" pattern="#,###" />원</span>
												</div>
												<div class="allowance-line">
													<span>기타수당</span> <span><fmt:formatNumber
															value="${sal.salPlus}" pattern="#,###" />원</span>
												</div>
											</td>
											<!-- 공제 총액 -->
											<td class="deduct-total"><fmt:formatNumber
													value="${sal.deductTotal}" pattern="#,##0" />원</td>

											<!-- 실 지급액 -->
											<td class="real-pay" data-realpay="${sal.realPay}"><fmt:formatNumber
													value="${sal.realPay}" pattern="#,##0" />원</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
...
$(function () {
    ...
    $('#salTable').DataTable({
        ...
        info: false,
        columnDefs: [
            { targets: [1], orderable: false } // 지급 요약
        ],
        language: {
